...
try:
    ...
    # cursor.execute(
    #     "create table if not exists employment(id int auto_increment,stu_id varchar(13) unique comment '学号',natureofunit varchar(40) comment '单位性质',workplace varchar(40) comment '工作单位',primary key(id))comment '毕业去向表';")
    # try:
    #     cursor.execute(
    #         "create index index_id on employment (id);")
    #     cursor.execute(
    #         "create index index_stu_id on employment (stu_id);")
    # except:
    #     print("已存在相同索引")
    print('连接成功!初始化成功!')
except:
...
for excel_file in excel_files:
    ...
    data_group = pd.concat([id_data, natureofunit_data, workplace_data], axis=1)
    ...
    sql = "insert into {} (stu_id,natureofunit, workplace) values (%s,%s,%s) on duplicate key update natureofunit=values(natureofunit), workplace=values(workplace)".format(DatabaseInfo.employment)
    cursor.executemany(sql, data_group)
    mydb.commit()

sql="delete from {} where stu_id not in (select stu_id from {});".format(DatabaseInfo.employment,DatabaseInfo.student)
cursor.execute(sql)
mydb.commit()

sql = "select natureofunit from {}".format(DatabaseInfo.employment)
